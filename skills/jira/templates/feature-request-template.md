...
```bash
# Create the feature request using jira-cli
jira issue create -p PROJ \
  -t Story \
  -s "Bulk export functionality for multiple projects" \
  -yHigh \
  -l feature-request -l export -l productivity

# With description from file (use -b for inline or editor opens)
jira issue create -p PROJ \
  -t Story \
  -s "Bulk export functionality for multiple projects" \
  -yHigh
