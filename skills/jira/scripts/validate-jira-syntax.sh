...
validate_file() {
  ...
  # Jira uses {code} for both open AND close (not {/code})
  # So count should be even: {code:java}...{code} = 2 occurrences
  local code_count=$(echo "$content" | grep -o "{code" | wc -l | tr -d ' ')
  if [ $((code_count % 2)) -ne 0 ]; then
    warning "Potential unclosed {code} tag (odd number: $code_count occurrences)"
  fi

  # Check for unclosed {panel} blocks
  # Jira uses {panel} for both open AND close (not {/panel})
  local panel_count=$(echo "$content" | grep -o "{panel" | wc -l | tr -d ' ')
  if [ $((panel_count % 2)) -ne 0 ]; then
    warning "Potential unclosed {panel} tag (odd number: $panel_count occurrences)"
  fi
